const dotenv = require('dotenv');
dotenv.config();
const cron = require('node-cron');
const connection = require('../config/db');

const now = new Date();
const startDate = new Date(now.getFullYear(), now.getMonth(), 1).toISOString().slice(0, 19).replace('T', ' ');
const endDate = new Date().toISOString().slice(0, 19).replace('T', ' ');

const key = (code_id, units = null) => `${code_id}_${units === null ? 'null' : units}`;

const isCptCodeBilled = async (patient_id, code_id, units = null) => {
  let sql = `SELECT 1 FROM cpt_billing WHERE patient_id = ? AND cpt_code_id = ? AND created BETWEEN '${startDate}' AND '${endDate}'`;
  const params = [patient_id, code_id];

  if (units !== null) {
    sql += ` AND code_units = ?`;
    params.push(units);
  }

  const [rows] = await connection.execute(sql, params);
  return rows.length > 0;
};
const checkMinutesForPatient = async () => {
  try {


    const [rpmPatients] = await connection.execute(`
      SELECT um.user_id AS patient_id,up.service_type
      FROM users_mappings um
      LEFT JOIN user_profiles up ON up.fk_userid = um.user_id
      WHERE um.fk_role_id = 7
      GROUP BY um.user_id
    `);
    const queries = [];
    for (const patient of rpmPatients) {
      const { patient_id, service_type } = patient;
      const pushedCptCodes = new Set();
      console.log("Billing for service type", service_type, "for patient", patient_id)

      if (service_type?.includes(1) && !service_type?.includes(2) && !service_type?.includes(3)) { //rpm
        const [rows] = await connection.query(
          `
          SELECT CEIL((
            (SELECT IFNULL(SUM(duration), 0)
             FROM patient_billing_notes
             WHERE patient_id = ?
               AND created BETWEEN ? AND ?) +
            (SELECT IFNULL(SUM(duration), 0)
             FROM tasks
             WHERE patient_id = ? 
               AND created BETWEEN ? AND ?)
          ) / 60) AS total_minutes
          `,
          [patient_id, startDate, endDate, patient_id, startDate, endDate]
        );

        const total_minutes = rows[0].total_minutes;
        console.log("total_minutes", total_minutes, patient_id);
        if (total_minutes >= 20 && total_minutes < 40) {
          // 99457 only
          const key = (code_id, units = null) => `${code_id}_${units === null ? 'null' : units}`;
          if (!pushedCptCodes.has(key(6)) && !await isCptCodeBilled(patient_id, 6)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
              params: [patient_id, 6]
            });
            pushedCptCodes.add(key(6));
          }
        } else if (total_minutes >= 40 && total_minutes < 60) {
          // 99457 + 99458 (1 unit)
          if (!pushedCptCodes.has(key(6)) && !await isCptCodeBilled(patient_id, 6)) {
            queries.push(
              {
                sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
                params: [patient_id, 6]
              });
            pushedCptCodes.add(key(6));
          }
          if (!pushedCptCodes.has(key(7, 1)) && !await isCptCodeBilled(patient_id, 7, 1)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id, code_units) VALUES (?, ?, ?)`,
              params: [patient_id, 7, 1]
            });
            pushedCptCodes.add(key(7, 1));
          }
        } else if (total_minutes >= 60) {
          // 99457 + 99458 (n units)
          if (!pushedCptCodes.has(key(6)) && !await isCptCodeBilled(patient_id, 6)) {
            queries.push(
              {
                sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
                params: [patient_id, 6]
              });
            pushedCptCodes.add(key(6));
          }
          if (!pushedCptCodes.has(key(7, 2)) && !await isCptCodeBilled(patient_id, 7, 2)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id, code_units) VALUES (?, ?, ?)`,
              params: [patient_id, 7, 2]
            });
            pushedCptCodes.add(key(7, 2));
          }
        }
        for (const query of queries) {
          await connection.execute(query.sql, query.params);
        }
      } else if (service_type?.includes(1) && service_type?.includes(2) && !service_type?.includes(3)) { //rpm+ccm
        const [rows] = await connection.query(
          `
          SELECT CEIL((
            (SELECT IFNULL(SUM(duration), 0)
             FROM patient_billing_notes
             WHERE patient_id = ?
               AND created BETWEEN ? AND ?) +
            (SELECT IFNULL(SUM(duration), 0)
             FROM tasks
             WHERE patient_id = ? 
               AND created BETWEEN ? AND ?)
          ) / 60) AS total_minutes
          `,
          [patient_id, startDate, endDate, patient_id, startDate, endDate]
        );

        const total_minutes = rows[0].total_minutes;
        console.log("total_minutes", total_minutes, patient_id);
        if (total_minutes >= 20 && total_minutes < 40) {
          // 99457 only
          const key = (code_id, units = null) => `${code_id}_${units === null ? 'null' : units}`;
          if (!pushedCptCodes.has(key(6)) && !await isCptCodeBilled(patient_id, 6)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
              params: [patient_id, 6]
            });
            pushedCptCodes.add(key(6));
          }
          if (!pushedCptCodes.has(key(10)) && !await isCptCodeBilled(patient_id, 10)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
              params: [patient_id, 10]
            });
            pushedCptCodes.add(key(10));
          }
        } else if (total_minutes >= 40 && total_minutes < 60) {
          // 99457 + 99458 (1 unit)
          if (!pushedCptCodes.has(key(6)) && !await isCptCodeBilled(patient_id, 6)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
              params: [patient_id, 6]
            });
            pushedCptCodes.add(key(6));
          }
          if (!pushedCptCodes.has(key(7, 1)) && !await isCptCodeBilled(patient_id, 7, 1)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id, code_units) VALUES (?, ?, ?)`,
              params: [patient_id, 7, 1]
            });
            pushedCptCodes.add(key(7, 1));
          }
          if (!pushedCptCodes.has(key(10)) && !await isCptCodeBilled(patient_id, 10)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
              params: [patient_id, 10]
            });
            pushedCptCodes.add(key(10));
          }
          if (!pushedCptCodes.has(key(3, 1)) && !await isCptCodeBilled(patient_id, 3, 1)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id, code_units) VALUES (?, ?, ?)`,
              params: [patient_id, 3, 1]
            });
            pushedCptCodes.add(key(3, 1));
          }

        } else if (total_minutes >= 60) {
          // 99457 + 99458 (n units)

          if (!pushedCptCodes.has(key(6)) && !await isCptCodeBilled(patient_id, 6)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
              params: [patient_id, 6]
            });
            pushedCptCodes.add(key(6));
          }
          if (!pushedCptCodes.has(key(7, 2)) && !await isCptCodeBilled(patient_id, 7, 2)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id, code_units) VALUES (?, ?, ?)`,
              params: [patient_id, 7, 2]
            });
            pushedCptCodes.add(key(7, 2));
          }
          if (!pushedCptCodes.has(key(10)) && !await isCptCodeBilled(patient_id, 10)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
              params: [patient_id, 10]
            });
            pushedCptCodes.add(key(10));
          }
          if (!pushedCptCodes.has(key(3, 2)) && !await isCptCodeBilled(patient_id, 3, 2)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id, code_units) VALUES (?, ?, ?)`,
              params: [patient_id, 3, 2]
            });
            pushedCptCodes.add(key(3, 2));
          }
        }
        for (const query of queries) {
          await connection.execute(query.sql, query.params);
        }

      } else if (service_type?.includes(1) && !service_type?.includes(2) && service_type?.includes(3)) { //rpm+pcm
        const [rows] = await connection.query(
          `
          SELECT CEIL((
            (SELECT IFNULL(SUM(duration), 0)
             FROM patient_billing_notes
             WHERE patient_id = ?
               AND created BETWEEN ? AND ?) +
            (SELECT IFNULL(SUM(duration), 0)
             FROM tasks
             WHERE patient_id = ? 
               AND created BETWEEN ? AND ?)
          ) / 60) AS total_minutes
          `,
          [patient_id, startDate, endDate, patient_id, startDate, endDate]
        );

        const total_minutes = rows[0].total_minutes;
        console.log("total_minutes", total_minutes, patient_id);
        if (total_minutes >= 30 && total_minutes < 60) {
          const key = (code_id, units = null) => `${code_id}_${units === null ? 'null' : units}`;
          if (!pushedCptCodes.has(key(1)) && !await isCptCodeBilled(patient_id, 1)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
              params: [patient_id, 1]
            });
            pushedCptCodes.add(key(1));
          }
        }

        if (total_minutes >= 20 && total_minutes < 40) {
          // 99457 only
          const key = (code_id, units = null) => `${code_id}_${units === null ? 'null' : units}`;
          if (!pushedCptCodes.has(key(6)) && !await isCptCodeBilled(patient_id, 6)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
              params: [patient_id, 6]
            });
            pushedCptCodes.add(key(6));
          }
        } else if (total_minutes >= 40 && total_minutes < 60) {
          if (!pushedCptCodes.has(key(6)) && !await isCptCodeBilled(patient_id, 6)) {
            queries.push(
              {
                sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
                params: [patient_id, 6]
              });
            pushedCptCodes.add(key(6));
          }
          if (!pushedCptCodes.has(key(7, 1)) && !await isCptCodeBilled(patient_id, 7, 1)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id, code_units) VALUES (?, ?, ?)`,
              params: [patient_id, 7, 1]
            });
            pushedCptCodes.add(key(7, 1));
          }
        } else if (total_minutes >= 60) {
          // 99457 + 99458 (n units)

          if (!pushedCptCodes.has(key(6)) && !await isCptCodeBilled(patient_id, 6)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
              params: [patient_id, 6]
            });
            pushedCptCodes.add(key(6));
          }
          if (!pushedCptCodes.has(key(7, 2)) && !await isCptCodeBilled(patient_id, 7, 2)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id, code_units) VALUES (?, ?, ?)`,
              params: [patient_id, 7, 2]
            });
            pushedCptCodes.add(key(7, 2));
          };
          if (!pushedCptCodes.has(key(1)) && !await isCptCodeBilled(patient_id, 1)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
              params: [patient_id, 1]
            });
            pushedCptCodes.add(key(1));
          }
          if (!pushedCptCodes.has(key(2, 1)) && !await isCptCodeBilled(patient_id, 2, 1)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id, code_units) VALUES (?, ?, ?)`,
              params: [patient_id, 2, 1]
            });
            pushedCptCodes.add(key(2, 1));
          }
        }
        for (const query of queries) {
          await connection.execute(query.sql, query.params);
        }

      } else if (!service_type?.includes(1) && service_type?.includes(2) && !service_type?.includes(3)) { //ccm
        const [rows] = await connection.query(
          `
          SELECT CEIL((
            (SELECT IFNULL(SUM(duration), 0)
             FROM patient_billing_notes
             WHERE patient_id = ?
               AND created BETWEEN ? AND ?) +
            (SELECT IFNULL(SUM(duration), 0)
             FROM tasks
             WHERE patient_id = ? 
               AND created BETWEEN ? AND ?)
          ) / 60) AS total_minutes
          `,
          [patient_id, startDate, endDate, patient_id, startDate, endDate]
        );

        const total_minutes = rows[0].total_minutes;
        console.log("total_minutes", total_minutes, patient_id);
        if (total_minutes >= 20 && total_minutes < 40) {
          const key = (code_id, units = null) => `${code_id}_${units === null ? 'null' : units}`;
          if (!pushedCptCodes.has(key(10)) && !await isCptCodeBilled(patient_id, 10)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
              params: [patient_id, 10]
            });
            pushedCptCodes.add(key(10));
          }
        } else if (total_minutes >= 40 && total_minutes < 60) {
          if (!pushedCptCodes.has(key(10)) && !await isCptCodeBilled(patient_id, 10)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
              params: [patient_id, 10]
            });
            pushedCptCodes.add(key(10));
          }
          if (!pushedCptCodes.has(key(3, 1)) && !await isCptCodeBilled(patient_id, 3, 1)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id, code_units) VALUES (?, ?, ?)`,
              params: [patient_id, 3, 1]
            });
            pushedCptCodes.add(key(3, 1));
          }
        } else if (total_minutes >= 60) {
          if (!pushedCptCodes.has(key(10)) && !await isCptCodeBilled(patient_id, 10)) {
            queries.push(
              {
                sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
                params: [patient_id, 10]
              });
            pushedCptCodes.add(key(10));
          }
          if (!pushedCptCodes.has(key(3, 2)) && !await isCptCodeBilled(patient_id, 3, 2)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id, code_units) VALUES (?, ?, ?)`,
              params: [patient_id, 3, 2]
            });
            pushedCptCodes.add(key(3, 2));
          }
        }
        for (const query of queries) {
          await connection.execute(query.sql, query.params);
        }
      } else if (!service_type?.includes(1) && !service_type?.includes(2) && service_type?.includes(3)) { //pcm
        const [rows] = await connection.query(
          `
          SELECT CEIL((
            (SELECT IFNULL(SUM(duration), 0)
             FROM patient_billing_notes
             WHERE patient_id = ?
               AND created BETWEEN ? AND ?) +
            (SELECT IFNULL(SUM(duration), 0)
             FROM tasks
             WHERE patient_id = ? 
               AND created BETWEEN ? AND ?)
          ) / 60) AS total_minutes
          `,
          [patient_id, startDate, endDate, patient_id, startDate, endDate]
        );

        const total_minutes = rows[0].total_minutes;
        console.log("total_minutes", total_minutes, patient_id);
        if (total_minutes >= 30 && total_minutes < 60) {
          const key = (code_id, units = null) => `${code_id}_${units === null ? 'null' : units}`;
          if (!pushedCptCodes.has(key(1)) && !await isCptCodeBilled(patient_id, 1)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
              params: [patient_id, 1]
            });
            pushedCptCodes.add(key(1));
          }
        }
        if (total_minutes >= 60) {
          if (!pushedCptCodes.has(key(1)) && !await isCptCodeBilled(patient_id, 1)) {
            queries.push(
              {
                sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id) VALUES (?, ?)`,
                params: [patient_id, 1]
              });
            pushedCptCodes.add(key(1));
          }
          if (!pushedCptCodes.has(key(2, 1)) && !await isCptCodeBilled(patient_id, 2, 1)) {
            queries.push({
              sql: `INSERT INTO cpt_billing (patient_id, cpt_code_id, code_units) VALUES (?, ?, ?)`,
              params: [patient_id, 2, 1]
            });
            pushedCptCodes.add(key(2, 1));
          }
          for (const query of queries) {
            await connection.execute(query.sql, query.params);
          }

        }
      }
      // After preparing queries for this patient, log the set of CPT codes being billed
      if (pushedCptCodes.size > 0) {
        console.log(`Pushed CPT codes for patient_id: ${patient_id}:`, Array.from(pushedCptCodes));
      } else {
        console.log(`No new CPT codes pushed for patient_id: ${patient_id}`);
      }

      const DeleteQ = `
      DELETE t1 FROM cpt_billing t1
      INNER JOIN cpt_billing t2 
        ON t1.id > t2.id
        AND t1.patient_id = t2.patient_id
        AND t1.cpt_code_id = t2.cpt_code_id
        AND IFNULL(t1.code_units, 0) = IFNULL(t2.code_units, 0)
        AND DATE(t1.created) = DATE(t2.created)
      WHERE t1.patient_id = ?
    `;
    await connection.execute(DeleteQ, [patient_id]);
      
    }

  } catch (err) {
    console.error('Cron job failed:', err.message);
  }
};
// checkMinutesForPatient();
cron.schedule('*/5 * * * *',getAllDevices);